...
# This code draws a circle, then draws marks (dot and label provided below in  list beside the dot)
...
vajra_mantra_cicle <- function (equalspacing=F) {
  ...
  p <- p +
    geom_point(data = marks_data[1:7], aes(x, y), size = 3, color = "green") +
    geom_text(data = marks_data[1:7], aes(x = 1.2 * x, y = 1.2 * y, label = label), color = "green")
